<?php 
    ...
    $sql_query = "SELECT movies.id, tmdb_id, title, director, year, production_company, plot, cover, GROUP_CONCAT(genre) as genre FROM movies RIGHT JOIN genre_movies ON movies.tmdb_id = genre_movies.movieID RIGHT JOIN genres ON genre_movies.`genreID` = genres.id  GROUP BY title";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $prod = $row['production_company'];
        // split genres
        $genres = $row['genre'];

        $pattern = '/[^,]+/';
        preg_match($pattern, $genres, $matches);
                ...
                <div class='card-body'>
                        ...
                        <div id='accordion'>
                            ...
                            <div id=heading'One>
                                ...
                                <div id='info-",$row['tmdb_id'],"'","class='collapse pt-4' aria-labelledby='headingOne' data-parent='#accordion'>
                                    <p>
                                    ",$row['plot'],"
                                    </p>
                                    ...
                                    foreach ($matches as $genre) {
                                        echo " <span class='badge badge-primary' data-toggle='tooltip' data-placement='bottom' title='Genre'>$genre</span>
                                        ";
                                    }
                                echo " </div>
                                </div>
